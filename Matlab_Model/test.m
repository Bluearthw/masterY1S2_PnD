...
charmsg = char(message);
charmsgPlus1 = char(message) + 1;
tablemsg = table(char(message) + 1);
joinmsg = join(table(char(message) + 1), "00");
char00msg = char("00" + join(table(char(message) + 1), "00") + "00");
charBefore48 = char("00" + join(table(char(message) + 1), "00") + "00")';
bitsout = char("00" + join(table(char(message) + 1), "00") + "00")' - 48;
%%
% decode
charin = char(bitsout+48);% +48 then to char
str = string(charin');% make it row!

substrings = split(str, '00');
substrings = substrings(2:end-1,:);%remove blank


lenMsg = length(substrings);
chars = zeros(1,lenMsg);
for i = 1:length(substrings)
    ascii = find(table == substrings(i));
    chars(i) = ascii;
