...
[gt,qt] = gauss_filter(Bb,Tb,fs,Nsamp);
 
%计算相位
phase    = zeros(1,Num*Nsamp);
phase(1) = Dsamp(1) * pi/2/Nsamp;
 
Dsamp    = conv(Dsamp,qt,'same');
...
figure;
subplot(421);plot(Imsk2);title('IGmsk');
subplot(423);plot(CARRIERI);title('cos');
subplot(425);plot(Imsk);title('IGmsk * cos');
 
 
 
subplot(422);plot(Qmsk2);title('QGmsk');
subplot(424);plot(CARRIERQ);title('sin');
subplot(426);plot(Qmsk);title('QGmsk * sin');
 
 
subplot(4,2,[7,8]);
plot(Ymsk);title('GMSK');
...
figure % 创建图形窗口
subplot(211)
stairs(data); title('原数据'); ylim([-1.2,1.2]);
subplot(212)
stairs(y); title('GMSK解调输出数据'); ylim([-1.2,1.2]);
figure % 创建图形窗口
