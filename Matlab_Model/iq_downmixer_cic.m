...
function complex_envelope = iq_downmixer_cic(signal, osr, br, fc_nom, fs)
    ...
    for offset = sweep_range
        ...
        complex_envelope = custom_cic_decim(mixed,R,N,plot_flag);
        TAP = 31;                       % Filter order (32 taps total)
        fc = 0.125;                   % Normalized cutoff frequency (0.125 × fs)
        h = fir1(TAP, fc, hamming(TAP+1)); % Use a Hamming window for good side-lobe suppression
        complex_envelope = conv(complex_envelope, h, 'same');
       
        % Power estimation (now AFTER LPF)
        energy = sum(abs(complex_envelope(1:64)).^2);

        fprintf('Current carrier frequency tried: %.2f Hz\n', fc_try);
        fprintf('Current energy found: %.2f\n', energy);
        fprintf('Current energy (hex) : 0x%X\n', uint64(energy));
    ...
    fprintf('Best energy found: %.2f\n', best_energy);
    fprintf('Best energy (hex) : 0x%X\n', uint64(best_energy));
    ...
    complex_envelope = custom_cic_decim(baseband_signal,R,N,plot_flag);

end 

function filtered_signal = custom_cic_decim(signal, R, N,plot_flag)
