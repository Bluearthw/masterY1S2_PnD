module dac_v2_tester (
	...
	*/
    ...
    always @(posedge clk) begin
		...
		r_readout <= (state_cur == 4'd3) ? ~vref_ctrl_cur : r_readout;
    end
    ...
    always @(*) begin
        ...
        case(state_cur)
            ...
            4'd1: begin // Start conversion
                vin_ctrl_nxt = 0;
                vref_ctrl_nxt = vref_ctrl_cur & ~(8'd1 << cnt_cur); // Try clearing current bit
                state_nxt = 4'd2;
				cnt_nxt <= cnt_cur;
            end

            4'd2: begin // Wait and evaluate comparator
                if (cmp == 1) begin
                    // Input > DAC
                    vref_ctrl_nxt = vref_ctrl_cur | (8'd1 << cnt_cur);
                end else begin
                    // Input <= DAC 
                    vref_ctrl_nxt = vref_ctrl_cur & ~(8'd1 << cnt_cur);
                end
